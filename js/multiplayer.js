...
class MultiplayerManager {
  ...
  openOnlineMenu() {
    if (typeof Peer === 'undefined') {
        alert("Erro: A biblioteca PeerJS não foi encontrada no HTML.");
        return;
    }
  ...
  sendGameAction(actionType, dataPayload) {
      ...
      if (this.isHost) {
          ...
          if (this.conn && this.conn.open) {
              ...
              console.warn("[Network] Sem conexão com o Host.");
          }
      }
  }
  ...
  processGameAction(message) {
      ...
      console.log(`[Sync] Ação processada: ${action}`, payload);

      // Dispara um Evento Global do Navegador para fácil escuta no script principal
      window.dispatchEvent(new CustomEvent('networkGameAction', { detail: { action, payload } }));

      switch(action) {
          case 'ROLL_DICE':
              if (typeof window.executeDiceRoll === 'function') window.executeDiceRoll(payload);
              break;
          case 'TRADE_OFFER':
              if (typeof window.executeTradeOffer === 'function') window.executeTradeOffer(payload);
              break;
          case 'TRADE_RESPONSE':
              if (typeof window.executeTradeResponse === 'function') window.executeTradeResponse(payload);
              break;
          case 'BUY_PROPERTY':
          ...
          case 'END_TURN':
              ...
              break;
          default:
              console.warn("[Sync] Ação não possui handler direto:", action);
      }
  }
  ...
  joinGame() {
    ...
    this.peer.on('error', (err) => {
      console.error("[PeerJS Error]", err);
      alert("Não foi possível encontrar a sala '" + hostId + "'. Verifique se o código está correto.");
    });
  }
